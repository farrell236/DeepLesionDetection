...
CLASSES = ['NA', 'Bone', 'Abdomen', 'Mediastinum', 'Liver', 'Lung', 'Kidney', 'Soft Tissue', 'Pelvis']
...
def visualize_detections(
    image, boxes, classes, scores, figsize=(7, 7), linewidth=1, color=[0, 0, 1], name=None
):
    ...
    plt.savefig(f'result_images/{name.replace("/", "_")}')
    plt.show()
    return ax
...
def resize_and_pad_image(
    ...
    # image, min_side=800.0, max_side=1024.0, jitter=[640, 960], stride=128.0
):
...
def tf_clip_and_normalize(tf_image, clip_min=-125, clip_max=255):
...
def preprocess_data2(sample):
    ...
    image = tf_clip_and_normalize(image) * 255

    bbox = tf.cast(sample["bbox"], dtype=tf.float32)
